<?php
$viewErrors = session()->getFlashdata('errors') ?? [];
?>
...
<div class="relative z-10 w-full max-w-xl bg-zinc-900/60 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl overflow-hidden ring-1 ring-white/10">
    <div class="w-full p-8 md:p-10 bg-transparent">
        <div class="max-w-md mx-auto">
            ...
            <form id="signupForm" method="post" action="<?= site_url('/register') ?>" onsubmit="handleSignup(event)" class="space-y-6">
                <?= csrf_field() ?>

                <!-- Username & Email -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="relative">
                        <input type="text" id="username" name="username" required value="<?= esc(old('username')) ?>" placeholder="Username"
                    ...
                    <div class="relative">
                        <input type="email" id="email" name="email" required value="<?= esc(old('email')) ?>" placeholder="Email"
                               class="peer w-full px-3 pt-6 pb-2 bg-zinc-800/50 border border-zinc-700 rounded-md text-sm text-white placeholder-transparent focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition hover:bg-zinc-800/70">
                ...
                <div class="grid grid-cols-1 gap-5">
                    <div class="relative">
                        <input type="text" id="firstname" name="firstname" required value="<?= esc(old('firstname')) ?>" placeholder="First Name"
                               class="peer w-full px-3 pt-6 pb-2 bg-zinc-800/50 border border-zinc-700 rounded-md text-sm text-white placeholder-transparent focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition hover:bg-zinc-800/70">
                    ...
                    <div class="relative">
                        <input type="text" id="lastname" name="lastname" required value="<?= esc(old('lastname')) ?>" placeholder="Last Name"
                               class="peer w-full px-3 pt-6 pb-2 bg-zinc-800/50 border border-zinc-700 rounded-md text-sm text-white placeholder-transparent focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition hover:bg-zinc-800/70">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="relative">
                        <input type="number" id="age" name="age" min="1" max="120" value="<?= esc(old('age')) ?>" placeholder="Age"
                               class="peer w-full px-3 pt-6 pb-2 bg-zinc-800/50 border border-zinc-700 rounded-md text-sm text-white placeholder-transparent focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary input-transition hover:bg-zinc-800/70">
