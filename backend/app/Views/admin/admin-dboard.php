...
$popularGenres = $popularGenres ?? [
    ['name' => 'Action', 'count' => 99],
    ['name' => 'RPG',    'count' => 99],
    ['name' => 'Strategy', 'count' => 99],
    ['name' => 'Simulation', 'count' => 99],
    ['name' => 'Puzzle', 'count' => 99],
];
                $status = $user['status'] ?? 'Inactive';
                // Map status to badge classes
                $statusMap = [
                    'Active'    => 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-500/20 text-green-400',
                    'Suspended' => 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-500/20 text-yellow-400',
                    'Inactive'  => 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-slate-700 text-gray-300',
                ];
                $statusClass = $statusMap[$status] ?? $statusMap['Inactive'];
...
<head>
    ...
    <title>Admin Dashboard with User Manager</title>
                ...
                <div class="container mx-auto px-4 lg:px-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                                <?php foreach ($popularGenres as $index => $genre): ?>
                                    <div class="flex justify-between items-center py-2 px-3 border-b border-gray-700">
                                        <span class="text-gray-300 font-medium"><?php echo esc(($index + 1) . '. ' . $genre['name']); ?></span>
                                        <span class="text-sm font-bold text-indigo-400"><?php echo esc($genre['count']); ?></span>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="bg-card-bg p-6 rounded-xl shadow-2xl border border-slate-600 mt-12">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <!-- Manage Games Card -->
                            <div
                                class="interactive-card bg-indigo-700 text-white p-8 md:p-10 rounded-xl shadow-lg transform transition-transform duration-300 ease-out hover:-translate-y-4 hover:scale-105 hover:shadow-2xl"
                                role="button" tabindex="0" aria-label="Manage Games" onclick="switchView('game-manager-view')">
                                <div class="flex flex-col h-full">
                                    ...
                                    <div class="mt-auto flex justify-end">
                                        <svg class="arrow-icon w-8 h-8 text-white" xmlns="http://www.w3.org/2000/svg" fill="none"
                            ...
                            <!-- Manage Users Card -->
                            <div
                                class="interactive-card bg-gray-700 text-white p-8 md:p-10 rounded-xl shadow-lg transform transition-transform duration-300 ease-out hover:-translate-y-4 hover:scale-105 hover:shadow-2xl"
                                role="button" tabindex="0" aria-label="Manage Users" onclick="switchView('user-manager-view')">
                                <div class="flex flex-col h-full">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-600" id="user-table">
                            ...
                            <tbody class="divide-y divide-slate-700">
                                <?php if (empty($users)): ?>
                                    <tr>
                                        <td class="px-3 py-4 text-sm text-gray-400" colspan="9">No users found.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($users as $user): ?>
                                        <tr class="hover:bg-slate-700/50">
                                            <td class="px-3 py-4 whitespace-nowrap text-sm font-medium text-white user-username"><?= esc($user['username'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-400 user-email"><?= esc($user['email'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-300 user-first-name"><?= esc($user['first_name'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-300 user-last-name"><?= esc($user['last_name'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-300 user-age-range"><?= esc($user['age_range'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm <?= ($user['account_type'] === 'Admin') ? 'text-indigo-400' : 'text-gray-300' ?> user-account-type"><?= esc($user['account_type'] ?? '') ?></td>
                                            <td class="px-3 py-4 whitespace-nowrap text-sm"><span class="<?= $statusClass ?>"><?= esc($status) ?></span></td>
                                            ...
                                            <!-- Keep a spare actions column if layout expects it -->
                                            <td class="px-3 py-4 whitespace-nowrap text-right text-sm font-medium"></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </tbody>
            ...
            <form id="edit-user-form" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="password" id="editUserPassword" name="editUserPassword" placeholder="Leave blank to keep current" class="w-full bg-slate-800 text-white px-3 py-2 rounded-lg border border-slate-700 focus:ring-accent focus:border-accent">
                    </div>
